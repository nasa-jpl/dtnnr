...
import {
  useInductLinkAllQuery,
  useReplaceInductLinkMutation,
} from '../utils/queries';
import { scrollElementIntoView, scrollToErrorSection } from '../utils/util';

export function InductLinksEdit({ inductId, hostId, closeModals }) {
  const initializedFormValues = useRef(false);
  const mutation = useReplaceInductLinkMutation(inductId);
  const [errorSection, setErrorSection] = useState('');
  const { isPending, isError, data, error } = useInductLinkAllQuery(inductId);
  const form = useForm({
    mode: 'uncontrolled',
      ...
      onError: (error) => {
        if (error.status === 404) {
          if (error.detail.includes('Induct ID')) {
            setErrorSection(
              `Induct ID ${inductId} does not exist. The induct` +
          ...
          switch (true) {
            case error.detail.includes('not associated with a host'):
              ...
              scrollToErrorSection();
              break;
            case error.detail.includes('LTP inducts cannot'):
              setErrorSection(
  ...
  return (
    <Box maw={500} mx="auto">
      ...
      <Text c="dimmed" size="sm">
        Select links to associate with the induct. The chosen links will{' '}
