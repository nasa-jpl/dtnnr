...
export function EndpointImcForm({
  initialEndpoint,
  nodeId,
  closeModals,
  mutation,
  ...
  const form = useForm({
    ...
    transformValues: (values) => ({
      ...values,
      disposition: values.disposition ? values.disposition : null,
    }),

    validate: {
      group_number: (value) =>
        value === ''
          ? `Group number may not be empty.`
  ...
  if (!initializedFormValues.current) {
    form.setValues({
      group_number: initialEndpoint?.group_number ?? '',
        ...
        if (error.status === 404) {
          ...
          switch (true) {
            ...
            case error.detail.includes(`already associated`):
              form.setFieldError('group_number', error.detail);
              scrollElementIntoView(form.getInputNode('group_number'));
              break;
            default:
