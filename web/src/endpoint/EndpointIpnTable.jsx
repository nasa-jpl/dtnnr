...
import { useDeleteEndpointIpnMutation } from '../utils/queries';
import { EndpointIpnCreate } from './EndpointIpnCreate';
import { EndpointIpnEdit } from './EndpointIpnEdit';
...
async function fetchEndpoints(nodeId, pageToken, pageSize) {
  ...
  const response = await fetch(`/api/nodes/${nodeId}/endpoints/ipn?${params}`);
  let result;
  try {
...
export function EndpointIpnTable({ nodeId, FQNN }) {
  const [pageSize, setPageSize] = useState(10);
  const [currentPageToken, setCurrentPageToken] = useState(undefined);
  const result = useQuery({
    queryKey: ['nodes', nodeId, 'ipn-endpoints', pageSize, currentPageToken],
  ...
  return (
    <EntityTable
      result={result}
      deleteRecord={useDeleteEndpointIpnMutation(nodeId)}
      fieldIdName="uri"
      columnsToShow={[
        {
          accessor: 'uri',
      ...
      createRecordName="Create ipn endpoint"
