...
from .schemas import (
    UnderlyingCommunicationServiceQueryRequest,
...
@get(
    path='/',
    dependencies={
        'query_request': Provide(
            UnderlyingCommunicationServiceQueryRequest, sync_to_thread=True
        )
    },
    sync_to_thread=True,
    tags=['links', 'reference'],
    summary='Query underlying communication services',
    description=(
        f'Search for and fetch underlying communication services.<br />'
        '<br />'
    ...
    responses={
        400: pagination_400_response_spec(
            f'{API_V1_PATH}/underlying-communication-services'
        )
    },
)
def query_underlying_communication_services(
    db_session: Session, query_request: UnderlyingCommunicationServiceQueryRequest
) -> QueryResponse[UnderlyingCommunicationServiceSchema]:
    path = '/underlying-communication-services'
    try:
...
underlying_communication_service_router = Router(
    path='/underlying-communication-services',
    route_handlers=[query_underlying_communication_services],
)
